...
class TestUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SteiStat Test Console")
    ...
    def _build_layout(self):
        ...
        top.pack(fill="x")
        ttk.Label(top, text="SteiStat Test Console", style="Header.TLabel").pack(anchor="w")
        ttk.Label(top, text="Set parameter, kirim command, monitor serial, dan plot data seperti workflow SteiStat EXE.").pack(anchor="w", pady=(2, 0))
        ...
        cmd_box = ttk.LabelFrame(content, text="SteiStat Commands", padding=12)
        ...
        ttk.Label(cv_panel, text="Start/Stop (V)").grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(cv_panel, text="Step(V)/Scan(V/s)/Cycle").grid(row=1, column=0, sticky="w", pady=(8, 0))
    ...
    def _apply_selected_method_params(self):
        ...
        try:
            if method == "CV":
                # UI fields are in volts / (V/s); firmware (RampTest.h) expects mV / (mV/s).
                start_mV = float(self.cv_start_var.get()) * 1000.0
                stop_mV = float(self.cv_stop_var.get()) * 1000.0
                step_mV = float(self.cv_step_var.get()) * 1000.0
                scan_mV_s = float(self.cv_scan_var.get()) * 1000.0
                cmd = f"D {start_mV},{stop_mV},{step_mV},{scan_mV_s},{int(float(self.cv_cycle_var.get()))}"
    ...
    def _run_selected_method(self):
        ...
        # Drop any leftover points from a previous run of this method so the
        # plot doesn't draw a stray line connecting the old run's last point
        # to the new run's first point (they're joined in arrival order).
        self._clear_mode_data(method)
        self._send_command(command)
    ...
    def _clear_mode_data(self, mode):
        self.mode_points.setdefault(mode, []).clear()
