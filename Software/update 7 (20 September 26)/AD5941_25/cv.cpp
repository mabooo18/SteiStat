...
static void _AD5940RampStructInit(void)
{
    ...
    // AppRAMPInit() only re-runs AppRAMPRtiaCal() when RAMPInited==bFALSE or
    // bParaChanged==bTRUE. RAMPInited latches TRUE after the first sweep and is
    // never cleared, so without this flag the LPTIA gain measured on the very
    // first sweep after power-up is reused for every later sweep - and a later
    // 'c <rcal>' command silently has no effect on the reported current.
...
static void _AD5940_Main(void)
{
    ...
    // Bound the acquisition loop. The sequencer asserts bTestFinished when the
    // ramp completes; if it never does (a dropped interrupt, a wedged AFE) this
    // loop previously spun forever and took the whole instrument with it, with
    // the USB stack still enumerating so the board merely looked idle. Allow
    // four times the nominal sweep duration plus a 30 s floor, then give up and
    // say so.
    const uint32_t sweep_ms = (uint32_t)(2000.0f * CycleNumber
                                         * fabsf(V_stop - V_start) / ScanRate);
    const uint32_t cv_deadline = millis() + 4u * sweep_ms + 30000u;

    // Loop polling interrupts until the sequencer asserts the finish flag
    while (!pRampCfg->bTestFinished)
    {
        if ((int32_t)(millis() - cv_deadline) >= 0)
        {
            Serial.println("ERR,CV_TIMEOUT");
