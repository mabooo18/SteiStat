...
void C_EIS::Run()
{
    ...
    if (m_pData->GetEIS_Mode() == 0) Utils_SetStatusLed(BLUE);   // Blue signals working cell impedance sweeps
    else Utils_SetStatusLed(RED);                           // Red signals internal resistor calibrations

    // Calculate logarithmic frequency steps
    float flo = (float)m_pData->GetFreqLo() / 1000.0f;
    float fhi = (float)m_pData->GetFreqHi() / 1000.0f;
    float log_start = log10(flo);
    float log_end = log10(fhi);
    float log_step = (log_end - log_start) / (m_pData->GetNFreqs() - 1);

    for (uint16_t i = 0; i < m_pData->GetNFreqs(); ++i)
    {
        ...
        Do_WaveGen(m_pData->GetEIS_Mode(), frequency, amplitude, tia_rf);
        ...
        if (m_pData->GetSeeedStatMode()) digitalWrite(D4, LOW);
        ...
        // Bounded wait: the DFT needs at most a few excitation periods, so allow
        // 8 periods plus a 250 ms floor. If it expires, the FIFO read below would
        // return a stale or empty value, so flag the point rather than silently
        // publishing it as a measurement.
        const uint32_t dft_timeout_ms = (uint32_t)(8000.0f / frequency) + 250;
        const bool dft_ok = Delay(&IsAD5940Calculating, 16, 0, 16, dft_timeout_ms);
        if (!dft_ok)
        {
            char wbuf[72];
            snprintf(wbuf, sizeof(wbuf), "EIS_TIMEOUT,%u,%.4f", (unsigned)(i + 1), frequency);
            Serial.println(wbuf);
        }
...
void C_EIS::RunSeeedStat()
{
    ...
    m_pData->SetEIS_Mode(0); // Sweep Rz
    Run();

    ResetMeasurementBuffer();
    m_pData->SetEIS_Mode(1); // Sweep Rcal
    Run();
...
{
    float* pRZ = m_pData->GetMeasurementsPtr();
    float* pRCAL = &m_pData->GetMeasurementsPtr()[2 * m_pData->GetNumberOfMeasurements()];

    for (int i = 0; i < m_pData->GetNumberOfMeasurements(); ++i)
    {
        float RZmag, RZphase;
