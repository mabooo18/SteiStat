...
/**
 * @brief Runs one full OCP session: configure, then measure. Matches the
 * Begin()/Run() shape every other method class already exposes, so
 * C_Communication::DispatchOCP() can dispatch OCP the same way it
 * dispatches CA/SWV/DPV/EIS.
 */
void C_OCP::Run()
{
    Configure();
...
void C_OCP::Configure()
{
    ...
    OutputPulse(D4, 300);
    if (m_pData->GetOCP_Calibration() == bTRUE)
    {
        digitalWrite(D4, HIGH);
        uint32_t hsDacDat = SetDACLevel(m_pData->GetWE_mV());
        digitalWrite(D4, LOW);
        ConfigureHSLoopCfg(hsDacDat);
    }

    // Toggle power switches to low-power buffers depending on calibration state
    BoolFlag enable = m_pData->GetOCP_Calibration() ? bTRUE : bFALSE;
...
void C_OCP::Measure()
{
    ...
    digitalWrite(D4, HIGH);
    if (m_pData->GetOCP_CalibrationCycling() == bTRUE)
    {
        // Sweep target voltages sequentially to calibrate equations
        float from = m_pData->GetWEFrom_mV();
        float to   = m_pData->GetWETo_mV();
        float step = from < to ? abs(m_pData->GetWEStep_mV()) : -abs(m_pData->GetWEStep_mV());
        for (float we = from; from < to ? we <= to : we >= to; we += step)
        {
            ...
            uint32_t ocp_sum = 0;
            for (uint16_t i = 0; i < m_pData->GetOCP_Npts(); ++i)
            {
                ocp_sum += Do1Measurement();
            }

            m_pData->SetOCP_Sum(ocp_sum);
            digitalWrite(D4, LOW);
        }
    }
    else
    {
        ...
        m_pData->SetOCP_Sum(0);
        for (uint16_t i = 0; i < m_pData->GetOCP_Npts(); ++i)
        {
            delayMicroseconds(500);
            m_pData->SetOCP_Sum(m_pData->GetOCP_Sum() + Do1Measurement());
        }
    }
    ...
    m_pData->SetADCCON(AD5940_ReadReg(REG_AFE_ADCCON));
    if (m_pData->GetOCP_CalibrationCycling() != bTRUE)
    {
        Serial.print('Z'); // Transmit completion token
    }

    m_pData->SetOCP_CalibrationCycling(false);
...
{
    float ocp_mV;
    float ocp_1 = static_cast<float>(m_pData->GetOCP_Sum()) / m_pData->GetOCP_Npts();
    if (m_pData->GetSeeedStatMode())
    {
        ocp_1 *= -1.0f; // Invert to represent Vre - Vwe instead of Vwe - Vre
    }

    if (m_pData->GetUseConstAB())
    {
        // Decode potential using calibrated slope and offset equations
        ocp_mV = (ocp_1 - m_pData->GetConstA()) / m_pData->GetConstB();
    }
    else
    {
        // Decode potential using direct physical registers math
        uint8_t GNPGA = (m_pData->GetADCCON() & BITM_AFE_ADCCON_GNPGA) >> BITP_AFE_ADCCON_GNPGA;
        ...
        uint8_t MUXSELN = (m_pData->GetADCCON() & BITM_AFE_ADCCON_MUXSELN) >> BITP_AFE_ADCCON_MUXSELN;
