  ...
  *pClocks = 0;
  ...
  switch(pFilterInfo->DataType)
  {
    case DATATYPE_ADCRAW:
      ...
      break;
    case DATATYPE_SINC3:
      sinc3osr = sinc3osr_table[pFilterInfo->ADCSinc3Osr];
      temp = (uint32_t)(((pFilterInfo->DataCount + 2) * sinc3osr + 1) * 20 * pFilterInfo->RatioSys2AdcClk + 0.5f);
      break;
    case DATATYPE_SINC2:
      sinc2osr = sinc2osr_table[pFilterInfo->ADCSinc2Osr];
      temp = (pFilterInfo->DataCount+1) * sinc2osr + 1;
...
{
   ENTER("ad5940.AD5940_SweepNext")
   float frequency;

   PRINTVAR("pSweepCfg->SweepLog", pSweepCfg->SweepLog);
   PRINTVAR("pSweepCfg->SweepStart", pSweepCfg->SweepStart);
   PRINTVAR("pSweepCfg->SweepStop", pSweepCfg->SweepStop);
   ...
   PRINTVAR("pSweepCfg->SweepPoints", pSweepCfg->SweepPoints)
   PRINTVAR("pSweepCfg->SweepIndex", pSweepCfg->SweepIndex)
   PRINTVAR("frequency", frequency)
   *pNextFreq = frequency;
   LEAVE
}
...
FreqParams_Type AD5940_GetFreqParameters(float freq)
{
	...
	// High power mode - mod here to get good performance out to 100 kHz - RJSM 111224
	if(freq > 101000)  // previously this was >= 20000, but HP mode is not actually needed till > 80 kHz
	{
		freq_params.DftSrc = DFTSRC_SINC3;
		freq_params.ADCSinc2Osr = 0;
		freq_params.ADCSinc3Osr = 2;    // doc: Oversampling rate of 2. Use this setting for the 400 kHz sinc3 filter output update rate and
                                        //      when the ADC update rate is 800 kSPS.
	...
	if(freq < 0.51)
	{
		...
		freq_params.ADCSinc3Osr = 1;    // doc: Oversampling rate of 4. Use this setting for the 400 kHz sinc3 filter output update rate and
                                        //      when the ADC update rate is 1.6 MSPS. High power option.
		freq_params.DftNum = DFTNUM_8192;
		freq_params.NumClks = 0;
		freq_params.HighPwrMode = bFALSE;   // this had been set TRUE; that is not consistent with the datasheet !!
		return freq_params;
	}

    // see AD5940-5941.pdf reg ADCFILTERCONV[ADCSAMPLERATE bit0] (page 59, table 43)
    uint32_t ADCFILTERCON = AD5940_ReadReg(REG_AFE_ADCFILTERCON);
    AdcRate = (ADCFILTERCON & BITM_AFE_ADCFILTERCON_ADCCLK) ? 800e3 : 16e6;
    Log(4, __LINE__, "(0x%X)REG_AFE_ADCFILTERCON=0x%X", REG_AFE_ADCFILTERCON, ADCFILTERCON);

	/* Start with SINC2 setting */
	for(uint8_t i = 0; i < sizeof(sinc2osr_table) / sizeof(uint32_t); i++)
	{
        // Sample rate after ADC filters
        n1 = sinc2osr_table[i] * sinc3osr_table[1];
        Log(0x40, __LINE__, "s2o=%i sinc2osr=%i sinc3osr=%i n1=%i fAdcRate=%.0f AdcRate/n1=%.2f, freq*10=%.2f", i, sinc2osr_table[i], sinc3osr_table[1], n1, AdcRate, (float)(AdcRate / n1), freq * 10);
        // aimed to use min 10 samples per period (below input freq of 20 kHz)
		...
		{
            // Sample rate after DFT block
            n2 = dft_table[j];
            iCycle = (uint32_t)(n1 * n2 * freq) / AdcRate;
            // The formula above (iCycle=...) is equivalent to this below: freq is divided by (ADC rate is divided by oversampling rate, and it is further divided by n2).
            // (n2 pieces of samples are used in one period of the incoming freq for DFT.) We need min 8 periods to be measured to get correct result.
            //uint32_t iCycle = (uint32_t)freq / (AdcRate / n1 / n2);
            Log(0x40, __LINE__, "dft=%i dftNum=%i iCycle=%i", j, n2, iCycle);
			if(iCycle < 8)
				continue;
			freq_params.DftSrc = DFTSRC_SINC2NOTCH;
			freq_params.ADCSinc2Osr = i - 1;    // why -1?
			freq_params.ADCSinc3Osr = 1;        // doc: Oversampling rate of 4. Use this setting for the 400 kHz sinc3 filter output update rate and
                                                //      when the ADC update rate is 1.6 MSPS. High power option.
            freq_params.DftNum = j;
			freq_params.NumClks = 0;
			freq_params.HighPwrMode = bFALSE;   // doc p55 ADC CIRCUIT OPERATION: For high power mode, the 32 MHz oscillator must be selected
                                                // as the ADC clock source. The ADC maximum update rate is 1.6 MSPS with higher power consumption, which is only required for
                                                // impedance measurements in the >80 kHz range.
            if(n1 == 4)                         // n1==4 only if i==0
			{
				freq_params.DftSrc = DFTSRC_SINC3;
				freq_params.ADCSinc2Osr = 0;    // 22 samples for this OSR setting
			}
			return freq_params;
		}
	}
...
void AD5940_WriteReg(uint16_t RegAddr, uint32_t RegData)
{
  ...
  if(SeqGenDB.EngineStart == bTRUE)
	...
	if (RegAddr == REG_AFE_AFECON)
	{
		delayMicroseconds(1);
	}

	PRINT2VARHEX(RegAddr, RegData)
	LEAVE
}
...
void AD5940_Initialize(void)
{
  ...
  for(unsigned i = 0; i < sizeof(RegTable) / sizeof(RegTable[0]); i++)
...
void AD5940_AFECtrlS(uint32_t AfeCtrlSet, BoolFlag State)
{
  ...
  PRINTVARHEX("REG_AFE_AFECON", AD5940_ReadReg(REG_AFE_AFECON))
  LEAVE
}
...
/**
   @brief Get switch matrix
   @return return SWMatrixCfg_Type filled with currect state of switch matrix.
*/
SWMatrixCfg_Type AD5940_SWMatrixCfgSGet()
{
    SWMatrixCfg_Type swMatrix;
    swMatrix.Dswitch = AD5940_ReadReg(REG_AFE_DSWFULLCON);
    swMatrix.Pswitch = AD5940_ReadReg(REG_AFE_PSWFULLCON);
    swMatrix.Nswitch = AD5940_ReadReg(REG_AFE_NSWFULLCON);
    swMatrix.Tswitch = AD5940_ReadReg(REG_AFE_TSWFULLCON);
    return swMatrix;
}
...
void AD5940_SWMatrixCfgS(SWMatrixCfg_Type *pSwMatrix)
{
    ...
    SWMatrixCfg_Type swm = AD5940_SWMatrixCfgSGet();
    Log(0x80, __LINE__, "REG_AFE_SWCON=0x%lX Dswitch=0x%X Nswitch=0x%x Pswitch=0x%X Tswitch=0x%X ",
          AD5940_ReadReg(REG_AFE_SWCON), swm.Dswitch, swm.Nswitch, swm.Pswitch, swm.Tswitch);
}
...
#ifdef CHIPSEL_M355
  AD5940_SetDExRTIA(1, pHsTiaCfg->HstiaDe1Rtia, pHsTiaCfg->HstiaDe1Rload);
#endif
...
void AD5940_WGCfgS(WGCfg_Type *pWGInit)
{
    ...
    if(pWGInit->WgType == WGTYPE_SIN)
    {
        ...
        //const float fACLK = 16e6;     // TODO change if needed (see e.g. AD5940-5941.pdf page 133)
        // see AD5940-5941.pdf reg WGFCW[SINEFCW 23:0](page 102, table 122) and reg ADCFILTERCONV[ADCSAMPLERATE bit0] (page 59, table 43)
        uint32_t ADCFILTERCON = AD5940_ReadReg(REG_AFE_ADCFILTERCON);
        float fACLK = (ADCFILTERCON & BITM_AFE_ADCFILTERCON_ADCCLK) ? 800e3 : 16e6;
        Log(2, __LINE__, "(0x%X)REG_AFE_WGFCW=%.2f Hz", REG_AFE_WGFCW, fACLK * (float)pWGInit->SinCfg.SinFreqWord / (1 << 30));
        Log(4, __LINE__, "(0x%X)REG_AFE_WGAMPLITUDE=0x%X", REG_AFE_WGAMPLITUDE, pWGInit->SinCfg.SinAmplitudeWord);
        Log(4, __LINE__, "(0x%X)REG_AFE_WGOFFSET=0x%X", REG_AFE_WGOFFSET, pWGInit->SinCfg.SinOffsetWord);
        Log(4, __LINE__, "(0x%X)REG_AFE_ADCFILTERCON=0x%X", REG_AFE_ADCFILTERCON, ADCFILTERCON);
    }
    ...
    else
    {
        ...
        Log(4, __LINE__, "(0x%X)REG_AFE_HSDACDAT=%ld(0x%X)", REG_AFE_HSDACDAT, pWGInit->WgCode, pWGInit->WgCode);
    }
    ...
    Log(4, __LINE__, "(0x%X)REG_AFE_WGCON=0x%X", REG_AFE_WGCON, tempreg);
}
...
void AD5940_WGFreqCtrlS(float SinFreqHz, float WGClock)
{
    ...
    Log(2, __LINE__, "(0x%X)REG_AFE_WGFCW=%.2f Hz", REG_AFE_WGFCW, SinFreqHz);
}
...
{
  ENTER("AD5940_LPDACCfgS")
  PRINTVAR("LpdacSel", pLpDacCfg->LpdacSel)
  PRINTVAR("DacData12Bit", pLpDacCfg->DacData12Bit)
  PRINTVAR("DacData6Bit", pLpDacCfg->DacData6Bit)
...
{
  ENTER("AD5940_LPDACWriteS")
  PRINTVAR("Data12Bit", Data12Bit)
  PRINTVAR("Data6Bit", Data6Bit)
...
{
  ENTER("AD5940_LPDAC0WriteS")
  PRINTVAR("Data12Bit", Data12Bit)
  PRINTVAR("Data6Bit", Data6Bit)

  Log(8, __LINE__, "Data12Bit=%i", Data12Bit);
  Log(8, __LINE__, "Data6Bit=%i", (uint16_t)Data6Bit * 64);
...
void AD5940_ADCFilterCfgS(ADCFilterCfg_Type *pFiltCfg)
{
    ...
    //*tempreg &= ~BITM_AFE_ADCFILTERCON_ADCCLK;
    ...
    */
    ...
    Log(4, __LINE__, "(0x%X)REG_AFE_ADCFILTERCON=0x%X", REG_AFE_ADCFILTERCON, tempreg);
...
void AD5940_DFTCfgS(DFTCfg_Type *pDftCfg)
{
    ...
    Log(4, __LINE__, "(0x%X)REG_AFE_ADCFILTERCON=0x%X", REG_AFE_ADCFILTERCON, reg_adcfilter);
    ...
    Log(4, __LINE__, "(0x%X)REG_AFE_DFTCON=0x%X", REG_AFE_DFTCON, reg_dftcon);
}
...
*/
uint32_t AD5940_TakeMeasurement(uint32_t afectrl_bits, uint32_t afectrl_readybit, uint32_t afectrl_resultType, int32_t *time_out)
{
    uint32_t ADCCode = 0;
    AD5940_INTCClrFlag(afectrl_readybit);
    AD5940_AFECtrlS(afectrl_bits, bTRUE);       /* Start conversion */
    do
    {
        AD5940_Delay10us(1);                    /* Delay 10us */
        if(AD5940_INTCTestFlag(AFEINTC_1, afectrl_readybit))
        {
            ADCCode = AD5940_ReadAfeResult(afectrl_resultType);
            break;
        }
    } while((*time_out)-- > 0);
    AD5940_AFECtrlS(afectrl_bits, bFALSE);      /* Stop conversion */
    return ADCCode;
}
...
static uint32_t __AD5940_TakeMeasurement(int32_t *time_out)
{
  ...
  return ADCCode;
#else
    return AD5940_TakeMeasurement(AFECTRL_ADCCNV | AFECTRL_SINC2NOTCH, AFEINTSRC_SINC2RDY, AFERESULT_SINC2, time_out);
#endif
}
...
AD5940Err AD5940_ADCPGACal(ADCPGACal_Type *pADCPGACal)
{
  ...
  uint32_t INTCCfg;
  int32_t ADCCode;
  BoolFlag bADCClk32MHzMode = bFALSE;       // MS: bADCClk32MHzMode was uninitialized!
...
AD5940Err AD5940_HSDACCal(HSDACCal_Type *pCalCfg)
{
    ...
    /* LSB_Numerator and LSB_Denominator are used to calculate
    ...
    /* Step1: Do offset calibration. */
    {
        ...
        ADCCode = (((ADCCode) * LSB_Denominator) / LSB_Numerator);
        if(ADCCode>0)
