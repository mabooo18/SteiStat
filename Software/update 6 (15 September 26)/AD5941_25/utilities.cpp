 ...
 */
bool Delay(bool (*func)(), int16_t redInc, int16_t greenInc, int16_t blueInc,
           uint32_t timeout_ms)
{
    int16_t red = 0;
    int16_t green = 0;
    ...
    // A predicate that never clears (e.g. a DFT interrupt that never fires
    // because the acquisition was mis-configured) used to wedge this loop
    // forever, taking the whole instrument down with no diagnostic. The wait
    // is now bounded and reports whether it completed or expired, so a caller
    // can abort the sweep and the host can see that it did.
    const uint32_t started = millis();
    const uint16_t step_ms = 2;   // was 100 ms - far coarser than the DFT it waits on
    ...
    return true;            // predicate cleared normally
}
