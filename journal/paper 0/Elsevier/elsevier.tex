...
\title{Design and Implementation of an Classification Potentiostat\\ With Performance Evaluation Using a Dummy Cell}
...
\author[1]{Kent Frumentius}
\author[2]{Isa Anshori}

\affiliation[1]{organization={Bandung Institute of Technology / School of Electrical Engineering and Informatics ITB},
            addressline={},
            city={Bandung},
            postcode={},
            state={},
...
\item Reports an honest, evidence-based audit of the firmware's classification  design maturity.
\end{highlights}
...
All measurements in this subsection were collected on the physical hardware described in Section~2: an AD5941 AFE on the \texttt{STEI\_Bstat\_v1.2} carrier board, a Seeed Studio XIAO RP2040 microcontroller, connected over USB-CDC serial at 1{,}000{,}000~baud, with a 3-branch resistive/capacitive dummy test cell (working, reference, and counter electrodes all connected), as documented in the project's internal hardware-verification log~\cite{ref:dummycell_log}. The dummy cell exposes three independently-wired branches from a shared CE/RE node, of which one branch is connected to the working-electrode sense pin (SE0) at a time:
\begin{itemize}
\item \textbf{we-A}: an anti-parallel small-signal diode pair (D1/D2, BAS16) in parallel with a 33~nF capacitor (C1), in series with a 560~$\Omega$ resistor (R1) --- the only nonlinear branch, intended as the closest analog on this board to a real redox reaction.
\item \textbf{we-B}: a single 10~k$\Omega$ (0.1\% tolerance) resistor (R2) --- purely ohmic.
\item \textbf{we-C}: a 10~k$\Omega$ resistor (R3) in parallel with a 33~nF capacitor (C2), in series with a 560~$\Omega$ resistor (R4) --- RC time constant $\approx$330~$\mu$s.
\end{itemize}
The we-A (diode/RC) branch was connected to SE0 for the chronoamperometry (Section~\ref{sec:ca_dummy}) and square-wave/differential-pulse voltammetry (Section~\ref{sec:swvdpv_dummy}) data reported below. The cyclic voltammetry data (Section~\ref{sec:cv_dummy}) were instead collected through the separate, unmodified legacy CV engine in an earlier test session; that data's near-ideal linear $I$--$V$ response (Section~\ref{sec:cv_dummy}) is consistent with a purely resistive branch (we-B) rather than we-A's nonlinear diode branch, but the exact branch connected for that session was not explicitly logged and is inferred here from the observed linearity rather than independently confirmed. All component values above are nominal design values; only R2 (we-B) has an independently documented tolerance (0.1\%), while the remaining resistor, capacitor, and diode tolerances on we-A were not re-verified against the bill of materials in this work.
...
with $V$ in mV and $I$ in $\mu$A, per the legacy engine's own unit convention: \texttt{AppRAMPDataProcess()} in \texttt{rampTest.cpp} stores the step potential directly in millivolts (derived from the 12-bit DAC LSB, \texttt{DAC12BITVOLT\_1LSB} $=2200/4095$~mV, itself matching Eq.~\eqref{eq:vbias}) and computes current as the ADC-sensed RTIA voltage (mV) divided by the calibrated RTIA magnitude ($\Omega$) and scaled by $10^{3}$, explicitly commented in the source as ``Result unit is uA.'' The near-unity coefficient of determination demonstrates that, for a purely resistive dummy-cell branch, the instrument's applied-potential and current-sensing chains behave linearly to within measurement noise across the full sweep range tested, which is the basic precondition for any subsequent accuracy/error-percentage analysis against $I=V/R$.
...
This subsection reports the instrument's response against a real electrolyte solution, complementing the dummy-cell results of Section~\ref{sec:eval_dummy} with genuine Faradaic/electrochemical behavior. \textbf{[MASUKIN DI SINI INFO PAKAI LARUTAN]}: solution measurements had not yet been collected/finalized at the time of this draft; the structure below mirrors Section~\ref{sec:eval_dummy} and should be completed with real data, figures, and captions before submission.

\subsubsection{Experimental Setup}
\textbf{[DATA LARUTAN]}: identity and concentration of the electrolyte/redox solution used (e.g. supporting electrolyte, redox couple), electrode materials (working/reference/counter), cell volume/geometry, and any temperature or stirring conditions, should be reported here.

\subsubsection{Chronoamperometry}
\textbf{[DATA LARUTAN]}: chronoamperometry current-transient data against the electrolyte solution, with applied potential step(s), and a figure analogous to Fig.~\ref{fig:ca_transient}.
...
\textbf{[DATA LARUTAN]}: cyclic voltammogram against the electrolyte solution, including peak current(s)/potential(s) if a redox-active analyte is present, and a figure analogous to Fig.~\ref{fig:cv}.
...
\subsubsection{Square-Wave and Differential Pulse Voltammetry}
\textbf{[DATA LARUTAN]}: SWV/DPV differential-current response against the electrolyte solution; unlike the dummy-cell result in Section~\ref{sec:eval_dummy}, a redox-active solution is expected to show a distinct voltammetric peak, and figures analogous to Figs.~\ref{fig:swv}--\ref{fig:dpv}.
...
\subsubsection{Open-Circuit Potential}
\textbf{[DATA LARUTAN]}: OCP measurement against the electrolyte solution, reported with the same averaging protocol used in Section~\ref{sec:eval_dummy}.
...
\item \textbf{Dummy-cell provenance.} The connected branch (we-A) and nominal per-branch component values are documented in Section~\ref{sec:setup_dummy} per the project's internal hardware-verification log; only R2's 0.1\% tolerance is independently documented, and tolerances for the remaining we-A components (R1, C1, D1/D2) were not re-verified against the bill of materials in this work.
...
Analog Devices, Inc., ``AD5940/AD5941 Ultra Low Power, High Precision Analog Front End for Electrochemical/Biopotential Systems, Data Sheet.'' Referenced throughout the project's firmware comments (e.g. \texttt{AD5940/AD5941.pdf});

\bibitem{ref:ad5940examples}
Analog Devices, Inc., ``ad5940-examples'' reference firmware repository, including the \texttt{AD5940\_ChronoAmperometric}, \texttt{AD5940\_SqrWaveVoltammetry}, and \texttt{AD5940\_Ramp} example projects used for comparison in this work. 
