...
\item Component footprints (Table~\ref{tab:bom}) are deliberately
  ...
  \item \textbf{Professional SMT assembly (recommended)}: submit the
    Gerbers together with \texttt{Production/bom-1.csv} and
    \texttt{Production/positions.csv} to a PCB assembly service; this
    covers U1 reliably and is the path
...
\item Connect the assembled board via USB; it enumerates as a standard
  RP2040 board.
\item Build and flash \texttt{AD5941\_25/AD5941\_25.ino} against the
  \texttt{rp2040:rp2040:seeed\_xiao\_rp2040} board target, e.g. with
  \texttt{arduino-cli}:
\begin{verbatim}
