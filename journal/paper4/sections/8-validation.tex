...
All eleven defects diagnosed below were found and fixed in this project's
own firmware -- \texttt{AD5941\_25.ino} and
\texttt{src/electrochemical\_methods/\{c\_ca,c\_swv,c\_dpv,dc\_potentiostat\_method\}.cpp},
\texttt{src/communication/communication.cpp}, \texttt{cv.cpp}, and
...
(\texttt{RTIACAL,<magnitude>,<phase>,<fRcal>}), surfaced nine further
defects (Section~\ref{sec:otherdefects}) not visible from the first pass
alone.
...
\subsection{Nine further defects}
\label{sec:otherdefects}
A second, deeper diagnostic pass surfaced nine additional defects,
summarized in Table~\ref{tab:otherdefects}. Several are individually more
consequential than Defects 1--2: D-RCAL in particular is a systematic
current-\emph{scale} error that a linearity fit cannot detect, because
$R^2$ is invariant under an affine rescaling of the measured variable --
it changes only how large the number is, not how straight the line
looks. Every one of D-PGA, D-VZEROBUF, D-SIGN, and D-WAKE existed in three
verbatim copies (Section~\ref{sec:oop}), plus a fourth, unreachable copy
in \texttt{electrochemical\_methods.cpp} confirmed dead via the live
command-dispatch table in \texttt{communication.cpp}; each had to be found
and fixed three times over.

\begin{table*}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Nine further defects found by the diagnostic method of
Section~\ref{sec:diagmethod}, beyond the two of
Sections~\ref{sec:rootcause}--\ref{sec:adcfix}.}
\label{tab:otherdefects}
\centering
\footnotesize
\begin{tabular}{@{}p{1.7cm}p{2.3cm}p{6.3cm}p{4.5cm}@{}}
\toprule
...
\label{sec:protocol}
All eleven defects were corrected, and the firmware was then rebuilt and
...
Beyond the eleven defects diagnosed above, the file-by-file comparison
...
described above after all eleven defects were fixed, immediately following
...
\emph{by reference comparison}, unlike CA and SWV, and remains subject to
